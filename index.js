...
client.query("select * from unnset(enum_range(null::categ_echipamente))",function(err,rez){
    console.log(rez);
})
...
app.use("/node_modules", express.static(__dirname+"/poze_uploadate"));
app.use("/node_modules", express.static(__dirname+"/node_modules"));//fac un alias
...
app.get("/produse",function(req,res){
    ...
    client.query("select * from unnset(enum_range(null::categ_prajituri))",function(err,rezOptiune){
        client.query(`select * from produse_iarna ${conditieQuery}`,function(err,rez){
        if(err){
            console,log(err);
            afisareEroare(res, 2);
        }
